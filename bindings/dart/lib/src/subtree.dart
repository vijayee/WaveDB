...
class Subtree implements Finalizable {
  ...
  dynamic _getSyncInternal(dynamic key) {
    ...
    try {
      ...
      // value (database_subtree_get_sync_raw returns NULL from
      // identifier_get_data_copy for zero-length values — that's success,
      // not a copy failure). Return an empty string, distinct from not-found
      // (rc == -2 -> null).
